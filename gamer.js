...
// Forzamos al proceso a no morir si hay un error de red
process.on('unhandledRejection', (reason, promise) => {
    console.log('⚠️ Error de red/API ignorado para seguir activo.');
});

const client = new Client({ 
    checkUpdate: false,
    patchVoice: false // Evita procesos innecesarios
});
...
client.on('ready', () => {
    ...
    console.log('✅ LISTO PARA EL ATAQUE');
    console.log('------------------------------------------');
});

client.on('messageCreate', async (msg) => {
    // 1. Si soy yo, no hago nada
    if (msg.author.id === client.user.id) return;

    // LOG PARA SABER SI EL BOT ESCUCHA (Míralo en Termux)
    console.log(`[LECTURA] Mensaje de: ${msg.author.username} en ID: ${msg.author.id}`);

    // 2. RESPUESTA A LA USUARIA ESPECIAL (Sin cooldown)
    if (msg.author.id === ID_USUARIA_SIN_PING) {
        if (msg.mentions.has(client.user.id) || (msg.reference && (await msg.channel.messages.fetch(msg.reference.messageId)).author.id === client.user.id)) {
            msg.channel.send(MENSAJE_SIN_PING).catch(() => {});
            return;
        }
    }

    // 3. RESPUESTA A OBJETIVOS
    if (OBJETIVOS.includes(msg.author.id)) {
        if (!cooldown) {
            cooldown = true;
            try {
                // Usamos send en el canal en vez de reply para ser más directos
                await msg.channel.send(MENSAJE_LARGO);
                console.log(`🔥 ENVIADO A: ${msg.author.username}`);
            } catch (err) {
                console.log(`❌ ERROR AL ENVIAR: ${err.message}`);
            }
            
            // Pausa de 1.5 segundos entre ataques para no morir por Rate Limit
            setTimeout(() => { cooldown = false; }, 1500);
        }
    }
});

client.login(TOKEN).catch(e => console.log("TOKEN INVÁLIDO"));
