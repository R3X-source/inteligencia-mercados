...
class MyClient(discord.Client):
    def __init__(self):
        # Desactivamos la carga de miembros y estados para que conecte rápido
        super().__init__(chunk_guilds_at_startup=False)
        self.ultima_respuesta = 0

    async def on_ready(self):
        print('-----------------------------------------')
        print(f'¡CONECTADO EXXITOSAMENTE COMO: {self.user}!')
        print(f'Vigilando a la ID: {ID_OBJETIVO}')
        print('-----------------------------------------')
    ...
    async def on_message(self, message):
        ...
        # Solo responder al objetivo
        if message.author.id == ID_OBJETIVO:
            ahora = time.time()
            if ahora - self.ultima_respuesta > ESPERA_SEGUNDOS:
                try:
