...
// Importamos las constantes de los intents
const { Intents } = require('discord.js-selfbot-v13');

const client = new Client({
    checkUpdate: false,
    // Activamos intents para Servidores (GUILDS) y Mensajes Directos
    intents: [
        'GUILDS', 
        'GUILD_MESSAGES', 
        'DIRECT_MESSAGES'
...
client.on('ready', () => {
    console.log(`Bot encendido. Respondiendo a ID: ${ID_USUARIO_OBJETIVO}`);
});

client.on('messageCreate', async (message) => {
    // Verificamos que sea el usuario objetivo
    ...
    if (transcurrido >= SEGUNDOS_ESPERA) {
        try {
            // Usamos message.channel.send() que es más estable en servidores que .reply()
            await message.channel.send(MENSAJE_DE_RESPUESTA);
            ultimoMensajeTime = Date.now();
            console.log(`Mensaje enviado en el canal: ${message.channel.name || 'DM'}`);
        } catch (err) {
            console.error('Error al enviar mensaje en servidor:', err.message);
        }
    }
});
