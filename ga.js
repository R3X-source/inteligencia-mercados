...
const client = new Client({ 
    checkUpdate: false,
    intents: [32767] // Máximos permisos para leer mensajes
});
...
client.on('ready', () => {
    ...
    console.log(`📡 ESCUCHANDO A ${OBJETIVOS.length} OBJETIVOS`);
    console.log('------------------------------------------');
});
...
client.on('messageCreate', async (msg) => {
    ...
    // 2. LOG DE CONTROL (Verás esto si alguien escribe)
    console.log(`📩 [MENSAJE DETECTADO] De: ${msg.author.username} | ID: ${msg.author.id}`);

    // 3. Comprobar si el autor está en la lista de OBJETIVOS
    if (OBJETIVOS.includes(msg.author.id)) {
        
        // Caso especial usuaria sin ping largo
        if (msg.author.id === ID_USUARIA_SIN_PING) {
            if (msg.mentions.has(client.user.id)) {
                await msg.channel.send(MENSAJE_SIN_PING).catch(() => {});
            }
            return;
        }

        // Ataque para el resto de objetivos
        try {
            await msg.channel.send(MENSAJE_LARGO);
            console.log(`🔥 [DISPARO] Respondido a ${msg.author.username}`);
        } catch (e) {
            console.log(`❌ [ERROR] No se pudo enviar: ${e.message}`);
        }
    }
});

client.login(TOKEN).catch(err => console.log("❌ ERROR DE TOKEN:", err.message));
