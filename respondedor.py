...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        # 1. Ignorar si el mensaje es nuestro
        if message.author.id == self.user.id:
            return
        
        # 2. Verificar si es el objetivo
        if message.author.id == ID_OBJETIVO:
            tiempo_actual = time.time()
            
            # 3. Control de tiempo (5 segundos)
            if tiempo_actual - self.ultima_respuesta > ESPERA_SEGUNDOS:
                try:
...
# Ejecución con reconexión automática
client = MyClient()
client.run(TOKEN)
