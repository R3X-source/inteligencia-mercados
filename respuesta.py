...
ID_OBJETIVO = 1458315274007744543  # ID numérica sin comillas
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        # Ignorar si no es el objetivo o si soy yo mismo
        if message.author.id != ID_OBJETIVO or message.author.id == self.user.id:
            return

        tiempo_actual = time.time()
        if tiempo_actual - self.ultima_respuesta > ESPERA:
            try:
                await message.channel.send(MENSAJE)
                self.ultima_respuesta = tiempo_actual
                print(f'Respondido en: {message.guild.name if message.guild else "DM"}')
            except Exception as e:
                print(f'Error al enviar: {e}')

# El bot se reconecta solo automáticamente con esta librería
client = MyClient()
client.run(TOKEN)
