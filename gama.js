...
client.on('ready', () => {
    console.log('------------------------------------------');
    console.log(`✅ CHAMA ACTIVO: ${client.user.tag}`);
    console.log(`📡 Esperando mensajes de los ${OBJETIVOS.length} objetivos...`);
    console.log('------------------------------------------');
});
...
client.on('messageCreate', async (msg) => {
    ...
    // 1. CASO ESPECIAL: ID_USUARIA_SIN_PING
    if (msg.author.id === ID_USUARIA_SIN_PING) {
        const meMenciono = msg.mentions.has(client.user.id);
        const esRespuesta = msg.reference && (await msg.channel.messages.fetch(msg.reference.messageId)).author.id === client.user.id;

        if (meMenciono || esRespuesta) {
            try {
                await msg.channel.send(MENSAJE_SIN_PING);
                console.log(`✨ Respuesta especial enviada a ${msg.author.username}`);
            } catch (e) {}
        }
        return; 
    }

    // 2. CASO OBJETIVOS GENERALES
    if (OBJETIVOS.includes(msg.author.id)) {
        // Cooldown de 2 segundos para no quemar la cuenta
        if (ahora - ultimoEnvio > 2000) {
            try {
                await msg.reply({ content: MENSAJE_LARGO });
                ultimoEnvio = ahora;
                console.log(`🔥 [SPAM] Respondido a: ${msg.author.username}`);
            } catch (e) {
                if (e.message.includes("rate limit")) {
                    console.log("⚠️ Discord aplicó Rate Limit. Esperando...");
                }
            }
        }
    }
...
client.login(TOKEN).catch(() => console.log("❌ ERROR: El TOKEN no funciona."));
