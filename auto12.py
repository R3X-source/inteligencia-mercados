...
# --- PARCHE MAESTRO PARA EL ERROR DE NONETYPE ---
# Esto sobreescribe la clase que causa el error 'NoneType object has no attribute get'
...
# Inyectamos el parche directamente en la librería cargada en memoria
discord.enums.FriendFlags = FriendFlags
discord.settings.FriendFlags = FriendFlags
# ------------------------------------------------
...
class MyClient(discord.Client):
    async def on_ready(self):
        print(f'\n>>> ¡LOGRADO! Conectado como: {self.user}')
        print(f'>>> Objetivo fijado: {ID_OBJETIVO}\n')

    async def on_message(self, message):
        if message.author.id == ID_OBJETIVO:
            try:
                await message.channel.send(MENSAJE)
                print(f'[!] Respuesta enviada en {message.guild.name if message.guild else "Privado"}')
                time.sleep(2) 
