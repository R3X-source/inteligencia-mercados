...
// CONFIGURACIÓN DE FUERZA BRUTA PARA LEER MENSAJES
const client = new Client({ 
    checkUpdate: false,
    intents: [32767], // Activa todos los privilegios
    restTimeOffset: 0
});
...
client.on('ready', () => {
    console.log(`\n🔥 BOT ACTIVO: ${client.user.tag}`);
    console.log(`🎯 LISTO PARA RESPONDER A ${OBJETIVOS.length} IDs\n`);
});

client.on('messageCreate', async (msg) => {
    // 1. FILTRO DE IDENTIDAD (Ignora a todos los que no están en la lista)
    if (!OBJETIVOS.includes(msg.author.id)) return;
    
    // 2. Ignorar mis propios mensajes
    if (msg.author.id === client.user.id) return;

    try {
        // CASO ESPECIAL USUARIA
        if (msg.author.id === ID_USUARIA_SIN_PING) {
            if (msg.mentions.has(client.user.id)) {
                await msg.channel.send("Neko fuck <@1384340161625591880>");
                console.log(`✨ Respondido a usuaria especial.`);
            }
        } 
        // CASO GENERAL OBJETIVOS
        else {
            await msg.channel.send(MENSAJE_LARGO);
            console.log(`🔥 DISPARO ENVIADO A: ${msg.author.username} (${msg.author.id})`);
        }
    } catch (e) {
