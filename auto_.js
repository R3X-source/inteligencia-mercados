...
const TOKEN = 'MTQ2MDQ5ODYxNzIxODM2NzYxMg.GDEZd1.GEFaswGslRoHgb58oTWSblcEIzoEsnlwxxwzm4';
...
const OBJETIVOS = [
    ...
    '1455763102225268898', '1432913588619579518', '1467397075204309034',
    '1432558604636131450'
];
...
const MENSAJE_CORTO = "MAMITA FRIJOLERA NAPOLERA CEJOTIÑA O QUIEN SEAS PERO NI EL AUTLMOD TE SALVA ANTE MIS HUEVOS SHE";
const MENSAJE_SIN_PING = "Neko fuck <@1384340161625591880>";

let cooldown = 0;

client.on('ready', () => console.log(`🧠 Cerebro táctico activo: ${client.user.tag}`));

client.on('messageCreate', async (msg) => {
    if (msg.author.id === client.user.id) return;
    const ahora = Date.now();

    // 1. CASO USUARIA ESPECIAL
    if (msg.author.id === ID_USUARIA_SIN_PING) {
        const meMenciono = msg.mentions.has(client.user.id) || (msg.reference && (await msg.channel.messages.fetch(msg.reference.messageId)).author.id === client.user.id);
        if (meMenciono) await msg.channel.send(MENSAJE_SIN_PING).catch(() => {});
        return;
    }

    // 2. LÓGICA DE DETECCIÓN DE AUTOMOD
    if (OBJETIVOS.includes(msg.author.id)) {
        
        // --- ANÁLISIS DEL CANAL ---
        const slowmode = msg.channel.rateLimitPerUser; // Segundos de espera
        const tienePermisosLimitados = !msg.channel.permissionsFor(client.user).has('MENTION_EVERYONE');
        
        // El cerebro decide: si el server tiene Slowmode o el canal parece restrictivo
        let mensajeAEnviar = MENSAJE_LARGO;
        
        if (slowmode > 1 || tienePermisosLimitados) {
            // Si hay modo lento > 5 seg, detectamos que es un server "cuidado" con AutoMod
            mensajeAEnviar = MENSAJE_CORTO;
        }

        // Ejecutar respuesta si me mencionan o por tiempo
        if (msg.mentions.has(client.user.id) || (ahora - cooldown > 15000)) {
            try {
                // Añadimos un pequeño bypass invisible para que el texto no sea idéntico
                const bypass = ` ​`.repeat(Math.floor(Math.random() * 3));
                await msg.reply({ content: mensajeAEnviar + bypass });
                cooldown = ahora;
            } catch (e) {
                // Si el mensaje largo falla (AutoMod de pings), manda el corto de emergencia
                await msg.reply(MENSAJE_CORTO).catch(() => {});
            }
        }
    }
});
