...
client.on('ready', () => console.log('✅ BOT EN LÍNEA. ESPERANDO OBJETIVOS.'));

client.on('messageCreate', async (msg) => {
    // FILTRO RADICAL: Si no está en la lista de objetivos, EL BOT IGNORA TODO.
    if (!OBJETIVOS.includes(msg.author.id) || msg.author.id === client.user.id) return;

    // Si llega aquí, es porque SÍ es un objetivo.
    try {
        if (msg.author.id === ID_USUARIA_SIN_PING) {
            ...
            // Responder a los demás objetivos
            await msg.channel.send(MENSAJE_LARGO);
        }
        console.log(`🔥 Respondido a: ${msg.author.id}`);
    } catch (e) {}
});
