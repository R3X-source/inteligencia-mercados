...
// Configuración con máxima prioridad de eventos
const client = new Client({ 
    checkUpdate: false,
    intents: [32767], 
    ws: { properties: { $os: 'Linux', $browser: 'discord.js', $device: 'discord.js' } }
});
...
client.on('ready', () => {
    console.log(`\n✅ SISTEMA EN LÍNEA: ${client.user.tag}`);
    console.log(`🎯 OBJETIVOS: ${OBJETIVOS.length} IDs en memoria.`);
...
client.on('messageCreate', async (msg) => {
    ...
    // 2. LOG OBLIGATORIO: Esto TIENE que salir en Termux si alguien escribe donde estés tú
    console.log(`[EVENTO] Mensaje recibido de ${msg.author.username} (${msg.author.id})`);

    // 3. Verificar si es objetivo
    if (OBJETIVOS.includes(msg.author.id)) {
        console.log(`🎯 ¡OBJETIVO DETECTADO! Intentando responder...`);
...
client.login(TOKEN).catch(e => console.error("❌ TOKEN INVÁLIDO:", e.message));
