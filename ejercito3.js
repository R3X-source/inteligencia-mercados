...
// 1. ELIMINACIÓN DE REPETIDOS: Usamos "new Set" para limpiar la lista automáticamente
const rawTokens = [
    ...
    'MTQ1ODY2ODk2OTI0NTQxMzM5NA.GoUl67.FhCsHSRgVzSJQMgW6xwUeWDbnWNQxLKEh-yLXg', // Estaba repetido
...
async function iniciarInstancia(token, index) {
    ...
    client.on('ready', async () => {
        console.log(`✅ [${index + 1}] ${client.user.tag} - ACTIVO`);

        const ejecutar = async () => {
            for (const id of targetIDs) {
                const finalMsg = generarVariante(baseMessage);
                try {
                    let channel = client.channels.cache.get(id) || await client.channels.fetch(id).catch(() => null);
                    if (channel) {
                        await channel.send(finalMsg);
                        console.log(`🚀 [${client.user.username}] -> Canal: ${id}`);
                    } else {
                        const user = await client.users.fetch(id).catch(() => null);
                        if (user) {
                            await user.send(finalMsg);
                            console.log(`👤 [${client.user.username}] -> MD: ${user.username}`);
                        }
                    }
                } catch (err) {
                    console.log(`⚠️ [${client.user.username}] Error en ID ${id}`);
                }
                // Pequeña espera entre mensajes de la misma cuenta para evitar flag
    ...
    // 2. MANEJO DE TOKENS INVÁLIDOS: El ".catch" evita que el script se rompa
    client.login(token).catch(() => {
        console.log(`❌ [TOKEN MUERTO] El token número ${index + 1} ya no sirve. Saltando...`);
    });
}

// Iniciar con un pequeño retraso entre logins para no saturar Termux
tokens.forEach((t, i) => {
    setTimeout(() => iniciarInstancia(t, i), i * 2000);
});
