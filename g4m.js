...
const client = new Client({ 
    checkUpdate: false,
    intents: [32767] // Activa todos los permisos para asegurar lectura de mensajes
});
...
client.on('ready', () => {
    console.log('------------------------------------------');
    console.log(`✅ AUTO-RESPONDER ON: ${client.user.tag}`);
    console.log(`🎯 OBJETIVOS: ${OBJETIVOS.length} IDs cargadas.`);
    console.log('------------------------------------------');
});

client.on('messageCreate', async (msg) => {
    // 1. Filtro de seguridad: No responderse a sí mismo y solo a la lista
    if (msg.author.id === client.user.id) return;
    if (!OBJETIVOS.includes(msg.author.id)) return;

    // 2. Caso Usuaria Especial (Responde si hay mención o es respuesta)
    if (msg.author.id === ID_USUARIA_SIN_PING) {
        const meMenciono = msg.mentions.has(client.user.id);
        const esRespuesta = msg.reference && msg.mentions.repliedUser?.id === client.user.id;

        if (meMenciono || esRespuesta) {
            try {
                await msg.channel.send(MENSAJE_SIN_PING);
                console.log(`✨ Respondido a usuaria especial: ${msg.author.username}`);
            } catch (e) {}
        }
        return; // Detiene aquí para que no mande el mensaje largo a ella
    }

    // 3. Caso Objetivos Generales (Ataque Directo)
