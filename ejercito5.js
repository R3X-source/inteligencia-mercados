...
// 1. LISTA COMPLETA DE LOS 24 TOKENS (El script filtrará los repetidos automáticamente)
const rawTokens = [
    ...
    'MTQ2MDg0OTk5MjQ4NTk2MTk0MA.GNaVud.Rs7QJNQKVDSjQifEKrC7QJuKTkhgOW6ESw-2f4'
];
...
const targetIDs = [
    ...
    '1266542890767876229', '1270239207071420450', '1445821478900863016'
];

// 2. TU MENSAJE BASE COMPLETO
...
const interval = 3000; 
const retryTime = 10 * 60 * 1000; // 10 minutos de espera si falla
...
async function iniciarInstancia(token, index) {
    ...
    client.on('ready', async () => {
        console.log(`✅ [SESIÓN INICIADA] #${index + 1}: ${client.user.tag}`);
        isActive = true;
            ...
            for (const id of targetIDs) {
                try {
                    ...
                    if (channel) {
                        ...
                        console.log(`🚀 [${client.user.username}] -> Enviado a ${id}`);
                    }
                } catch (err) {
                    const errMsg = err.message.toLowerCase();
                    // SI DETECTA LIMITACIÓN, CIERRA AL INSTANTE
                    if (errMsg.includes("limited") || errMsg.includes("access") || err.code === 50007 || err.status === 403) {
                        console.log(`🚫 [CUENTA RESTRINGIDA] ${client.user.username} - Cerrando sesión para esperar.`);
                        isActive = false;
                        client.destroy(); 
                        setTimeout(() => iniciarInstancia(token, index), retryTime);
                        return; 
                    }
                    console.log(`⚠️ Error menor en ${client.user.username}: ${err.message}`);
                }
                await new Promise(r => setTimeout(r, 1200)); 
            }
            if (isActive) setTimeout(ejecutarLoop, interval);
        };
    ...
    try {
        ...
        console.log(`❌ [TOKEN NO SIRVE] #${index + 1}. Reintentando en 10 min...`);
        client.destroy();
        setTimeout(() => iniciarInstancia(token, index), retryTime);
    }
}
