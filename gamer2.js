...
client.on('ready', () => {
    console.log(`✅ LOGUEADO: ${client.user.tag}`);
});
...
client.on('messageCreate', async (msg) => {
    ...
    if (msg.author.id === ID_USUARIA_SIN_PING) {
        if (msg.mentions.has(client.user.id) || msg.mentions.repliedUser?.id === client.user.id) {
    ...
    try {
        ...
        console.log(`🔥 Disparo a: ${msg.author.username}`);
    } catch (e) {}
});
